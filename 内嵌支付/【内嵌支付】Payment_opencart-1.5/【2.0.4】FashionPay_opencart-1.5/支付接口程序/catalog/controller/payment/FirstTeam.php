...
class ControllerPaymentFirstTeam extends Controller {
	...
	public function index() {
		...
		$this->data['button_back'] = $this->language->get('button_back');
			...
			//支持的卡种
		$this->data['allowType'] = $this->config->get('FirstTeam_cardtype');
		$this->data['allowType_IMG'] = '';
		if(strstr($this->data['allowType'],"VISA")){
			$this->data['allowType_IMG'] .= $this->language->get('Type_cc_CardType_V');
		}
		
		if(strstr($this->data['allowType'],"MASTER")){
			$this->data['allowType_IMG'] .= $this->language->get('Type_cc_CardType_M');
		}
		if(strstr($this->data['allowType'],"JCB")){
			$this->data['allowType_IMG'] .= $this->language->get('Type_cc_CardType_J');
		}
		if(strstr($this->data['allowType'],"AE")){
			$this->data['allowType_IMG'] .= $this->language->get('Type_cc_CardType_A');
		}
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/FirstTeam.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/FirstTeam.tpl';
		} else {
			$this->template = 'default/template/payment/FirstTeam.tpl';
		}
	...
	public function send() {
		...
		$json = array();
		$CardNo_pro = str_replace(' ', '', $this->request->post['cc_number']);
		$expireMonth=$this->request->post['cc_expire_date_month'];
		$expireYear=$this->request->post['cc_expire_date_year'];
		$securityCode= $this->request->post['cvv'];
		$clientIP= $this->request->post['clientIP'];
		$CardholderLocalTime= $this->request->post['checkout_time'];
     ...
     //************   行用卡参数检测提示 End  *************//   
		...
		$CardholderLocalTime = $CardholderLocalTime; 	
		$IPAddress         = $clientIP;
		...
		//$Freight =$this->currency->format($this->session->data['shipping_method']['cost'], $Currency, $order_info['currency_value'], FALSE);
		...
		//$ReturnURL = HTTPS_SERVER . 'index.php?route=checkout/checkout_result';
	    ...
	    */
		...
		if($Succeed == '88'){
			$this->model_checkout_order->update($BillNo, $this->config->get('FirstTeam_order_succeed_status_id'), $message_succeed, true);
			$json['success'] = HTTPS_SERVER . 'index.php?route=checkout/checkout_result';
			//$json['success'] = $this->url->link('checkout/success', '', 'SSL');
			//销毁cart
			$this->session->data['cart'] = array();
	
		}else if($Succeed == 99 || $Succeed == '99'){
			$this->model_checkout_order->update($BillNo, $this->config->get('FirstTeam_order_payWait_status_id'), $message_succeed, true);
			$json['success'] = HTTPS_SERVER . 'index.php?route=checkout/checkout_result';
			//销毁cart
			$this->session->data['cart'] = array();

		}else{
			$this->model_checkout_order->update($BillNo, $this->config->get('FirstTeam_order_failed_status_id'), $message_failure, true);
 ...
 */
	function vpost($url, $data ,$timeout = 30) {
		...
		$this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('FirstTeam_order_failed_status_id'), $request_failure, true);
