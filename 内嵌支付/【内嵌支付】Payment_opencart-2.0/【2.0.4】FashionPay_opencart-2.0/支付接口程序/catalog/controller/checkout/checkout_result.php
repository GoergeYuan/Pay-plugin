 ...
 //如遇到页面刷新，返回页面操作显示 Do not refresh!
	...
	// 判断成功和失败页面的字体颜色和信息
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/checkout_result.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/checkout/checkout_result.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/checkout/checkout_result.tpl', $data));
		}
	}
}